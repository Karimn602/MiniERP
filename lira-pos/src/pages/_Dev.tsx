    ...
    <div className="space-y-4">
      ...
      <Section title="Phase 2A — products / barcodes / VAT / UoM">
        <Btn disabled={busy} onClick={() => run("vatRatesRepo.listActive()", async () => {
          ...
          return rates.map((r) => ({
            name: r.name, rate: formatBps(r.rateBps), exempt: r.isExempt,
          }));
        })}>List VAT rates</Btn>
        ...
        <Btn disabled={busy} onClick={() => run("barcodes (olive oil)", async () => {
          const r = await barcodesRepo.listForProduct("00000000-0000-0000-0000-0000000000d3");
          return r.map((b) => ({ barcode: b.barcode, primary: b.isPrimary }));
        })}>List barcodes</Btn>

        <Btn disabled={busy} onClick={() => run("today's FX rate", async () => {
          try { return await exchangeRatesRepo.getCurrentForToday(storeId!); }
          catch (e) {
            if (e instanceof Error && e.message === "NO_EXCHANGE_RATE_SET") return "(none set)";
            throw e;
          }
        })}>Get FX rate</Btn>
      </Section>
      ...
      <Section title="Phase 2C — suppliers / purchases / inventory">
        <Btn disabled={busy} onClick={() => run("suppliersRepo.list()", async () =>
          (await suppliersRepo.list({ storeId: storeId! })).map((s) => ({
            name: s.name, contact: s.contactName, active: s.isActive,
          }))
        )}>List suppliers</Btn>

        <Btn disabled={busy} onClick={() => run("purchasesRepo.list()", async () =>
          (await purchasesRepo.list({ storeId: storeId! })).map((p) => ({
            no: p.purchaseNumber, date: p.purchaseDate, type: p.purchaseType,
            total: formatUsd(p.totalInclVatCents), status: p.status,
          }))
        )}>List purchases</Btn>

        <Btn disabled={busy} onClick={() => run("movementsRepo.listRecent()", async () =>
          (await movementsRepo.listRecent({ storeId: storeId!, limit: 20 })).map((m) => ({
            when: m.postedAt, type: m.movementType, delta: m.quantityDelta,
            unitCost: formatUsd(m.unitCostInclVatCents),
          }))
        )}>List recent movements</Btn>

        <Btn disabled={busy} onClick={async () => {
          // End-to-end smoke: create supplier, post 1-line purchase, verify stock+cost.
          await run("E2E: supplier + purchase + stock check", async () => {
            const supplier = await suppliersRepo.create({
              storeId: storeId!,
              name: `Probe Supplier ${Date.now()}`,
            });
            const products = await productsRepo.listEnriched({ storeId: storeId!, limit: 1 });
            if (products.length === 0) throw new Error("No products to test with.");
