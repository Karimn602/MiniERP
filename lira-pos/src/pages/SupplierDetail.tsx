  ...
  if (!supplier) {
    return (
      <div className="space-y-2">
        ...
        <Link to="/suppliers" className="text-sm text-brand underline">← Back to suppliers</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <Link to="/suppliers" className="text-xs text-brand hover:underline">← All suppliers</Link>
          <h2 className="mt-1 text-2xl font-semibold text-slate-900">{supplier.name}</h2>
      ...
      <div className="flex items-center gap-3">
        ...
        {justSaved && <span className="text-sm text-emerald-700">✓ Entry posted</span>}
      </div>
        ...
        {entries.length === 0 ? (
          <CardBody>
            <div className="py-8 text-center text-sm text-slate-500">
              No activity yet. Posting a purchase from this supplier — or a payment recorded here — will appear.
            </div>
          </CardBody>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {entries.map((e) => (
                  <tr key={e.id}>
                    <td className="px-5 py-2 text-slate-700">{e.entryDate}</td>
                    ...
                    <td className="px-5 py-2 text-xs text-slate-600">
                      {e.notes ?? "—"}
                      {e.relatedPaymentId && (
  <div className="text-[10px] text-slate-500">payment: {e.relatedPaymentId}</div>
)}
                    </td>
                    <td
                      className={clsx(
                        "px-5 py-2 text-right font-medium",
                        e.amountSignedCents > 0 ? "text-red-700" : "text-emerald-700",
                      )}
                    >
                      {e.amountSignedCents > 0 ? "+" : ""}{formatUsd(e.amountSignedCents)}
                    </td>
                  </tr>
                ))}
              </tbody>
...
function EntryBadge({ type }: { type: LedgerEntryType }) {
  const meta: Record<LedgerEntryType, { label: string; className: string }> = {
    purchase:        { label: "purchase",        className: "bg-slate-200 text-slate-700" },
    payment:         { label: "payment",         className: "bg-emerald-100 text-emerald-800" },
    credit_memo:     { label: "credit memo",     className: "bg-teal-100 text-teal-800" },
    opening_balance: { label: "opening balance", className: "bg-indigo-100 text-indigo-800" },
...
function RecordEntryForm({
  ...
  type Kind = "payment" | "credit_memo" | "opening_balance" | "adjustment";
  ...
  async function handleSubmit() {
    ...
    // Convert magnitude to signed amount per kind.
    // Convention: + = we owe more; − = we owe less.
    let signed: number;
    switch (kind) {
      case "payment":
        signed = -amountCents;   // paying down → we owe less
        break;
      case "credit_memo":
        signed = -amountCents;   // supplier credits us → we owe less
        break;
      case "opening_balance":
        signed = amountCents;    // typically positive = pre-existing AP
        break;
      case "adjustment":
  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-4">
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-700">Entry type</label>
          <div className="inline-flex rounded-md border border-slate-300 bg-white p-0.5 shadow-sm">
            {(["payment", "credit_memo", "opening_balance", "adjustment"] as const).map((k) => (
              <button
                key={k}
                type="button"
                onClick={() => setKind(k)}
                className={clsx(
                  ...
                  kind === k ? "bg-brand text-brand-fg" : "text-slate-600 hover:bg-slate-50",
                )}
              >
                {k === "payment" ? "Payment" :
                 k === "credit_memo" ? "Credit note" :
                 k === "opening_balance" ? "Opening balance" : "Adjustment"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              {kind === "adjustment" && (
                <div className="inline-flex rounded-md border border-slate-300 bg-white text-xs shadow-sm">
                  <button
                    ...
                    className={clsx(
                      "px-2 py-1.5",
                      adjustmentSign === "+" ? "bg-red-600 text-white" : "text-slate-600 hover:bg-slate-50",
                    )}
                  ...
                  <button
                    ...
                    className={clsx(
                      "px-2 py-1.5",
                      adjustmentSign === "-" ? "bg-emerald-600 text-white" : "text-slate-600 hover:bg-slate-50",
                    )}
          ...
          {(kind === "payment" || kind === "credit_memo") && (
            <Input
              label={kind === "payment" ? "Payment reference" : "Credit memo ref"}
